...
print(f"(Box = red)   = {p_red:.2f}")
print(f"(Box = blue)  = {p_blue:.2f}")
...
print(f"(F = apple)  = {p_a:.4f}  ({p_a*100:.2f}%)")
print(f"(F = orange) = {p_o:.4f}  ({p_o*100:.2f}%)\n")

print("=== Posterior probability ===")
print(f"(Box = red | F = orange) = {p_red_given_o:.4f}  ({p_red_given_o*100:.2f}%)\n")